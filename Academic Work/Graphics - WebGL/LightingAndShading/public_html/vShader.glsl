...
void main() {
    ...
    // Note that we could also to per-pixel Phong shading rather than Gouraud.
    ...
    // But we should not normalize these them since they are points and not vectors
    ...
    vec3 ambientComponent = ambientFactor * (lightColor * materialColor);
    ...
    //   Don't forget to check for back facing surfaces which should not get lit
    
    vec3 L = normalize(lightVC3 - posVC3);
    float RIF = dot(L, normVC3);
    vec3 diffuseComponent = vec3(0,0,0);
    if(RIF >= 0.0)
    {
        diffuseComponent = RIF * (materialColor * lightColor);
    }
    ...
    // Recall the specular intensity using Blinn's is: (H dot N)^Shniy
    
    // TODO: calculate the reflective intensity and apply it to find the reflective
    //   color using the material  and light colors
    //   Don't forget to check for back facing surfaces which should not get lit
    vec3 V = normalize(-posVC3);
    vec3 halfway = (L + V) / 2.0;
    RIF = pow(dot(halfway, normVC3), shiny);
    vec3 specularComponent = vec3(0,0,0);
    if(RIF >= 0.0)
    {
        specularComponent = RIF *(materialColor * lightColor);
    }
